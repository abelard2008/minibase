...
class O_buf {
 public:
   ...
   ~O_buf();				// Destructor
#ifdef TEST
void init(char **bufs, int n_pages, int tSize, int temp_fd, bool buffer = FALSE);
#else
void init(char **bufs, int n_pages, int tSize,
		 HeapFile* temp_fd, bool buffer = FALSE);
#endif
...
//   void init(char **bufs, int n_pages, int tSize, int temp_fd, bool buffer = FALSE);
 ...
 private:
   bool dirty;				// Does this buffer contain dirty pages?
