...
public:
    ...
    virtual Plan_Relation* GetPlan_Relation( const std::string& relName ) = 0;

      // Does the given relation exist?
    virtual bool RelationExists( const std::string& relName ) = 0;

      // This method creates a new Attribute object and returns it (or null).
      // It is the caller's responsibility to delete it.
    virtual Attribute* GetAttribute( const std::string& relName, const std::string& attrName ) = 0;
...
public:
    ii_BogusSystemCatalogs( const char* catname )
        : database(catname)
        {}
    ...
    virtual Plan_Relation* GetPlan_Relation( const std::string& relName );
    virtual bool RelationExists( const std::string& relName );
    virtual Attribute* GetAttribute( const std::string& relName, const std::string& attrName );
