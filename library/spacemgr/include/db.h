...
public:
    ...
    Status add_file_entry(const std::string fname, PageId start_page_num);

    // Delete the entry corresponding to a file from the header page(s).
    Status delete_file_entry(const std::string fname);

    // Get the entry corresponding to the given file.
    Status get_file_entry(const std::string name, PageId& start_pg);
