...
#ifdef DEBUG 
  std::cout<<"Creating database "<<fname<<" with pages "<<num_pgs<<endl; 
#endif
...
#ifdef BM_TRACE
    s = MINIBASE_BM->pinPage( 0, (Page*&)fp, true /*==empty*/, "*** DB admin ***" );
#else
...
DB::DB(const char* fname, Status& status)
{
...
#ifdef DEBUG
    std::cout << "opening database "<<fname<<endl;
#endif
    ...
    Status s;
    first_page* fp;
    num_pages = 1;      // We initialize it to this.  We will know the real size after we read page 0.

#ifdef BM_TRACE
    s = MINIBASE_BM->pinPage( 0, (Page*&)fp, false /*not empty*/, "*** DB admin ***" );
#else
...
{
#ifdef DEBUG
    std::cout<< "Closing database " << name <<endl;
#endif
    ::close( fd );
    fd = -1;
    ::free( name );
...
//*****************************************************
// This function destroys the database. That has simply the effect
...
{
#ifdef DEBUG
    std::cout<<"Destroying the database"<<endl;
#endif
...
{
#ifdef DEBUG
    std::cout << "Allocating a run of "<<run_size<<" pages."<<endl;
#endif
...
#ifdef DEBUG
        std::cout<<"Page allocated in get_free_pages:: "<<start_page_num<<endl;
#endif
...
{
#ifdef DEBUG
    std::cout << "Deallocating a run of " << run_size << " pages starting at "
         << start_page_num << std::endl;
#endif
...
Status DB::add_file_entry(const std::string fname, PageId start_page_num)
{
#ifdef DEBUG
    std::cout << "Adding a file entry:  "<<fname<<" : "<<start_page_num<<endl;
#endif

      // Is the info kosher?
if ( strlen(fname.c_str()) >= MAX_NAME )
    ...
    do {
        ...
        unsigned entry = 0;
        while ( entry < dp->num_entries && dp->entries[entry].pagenum != INVALID_PAGE )
            ++entry;
    ...
    if ( !found )  // Have to add a new header page if possible.
      {
    ...
    strcpy( dp->entries[free_slot].fname, fname.c_str() );
...
Status DB::delete_file_entry(const std::string fname)
{
#ifdef DEBUG
    std::cout << "Deleting the file entry for "<<fname<<endl;
#endif
    ...
    do {
        ...
        unsigned entry = 0;
        while ( entry < dp->num_entries && (dp->entries[entry].pagenum == INVALID_PAGE
                                            || 0 != strcmp(fname.c_str(),dp->entries[entry].fname)) )
            ++entry;
...
Status DB::get_file_entry(const std::string fname, PageId& start_page)
{
#ifdef DEBUG
    std::cout << "Getting the file entry for "<<fname<<endl;
#endif
...
{
#ifdef DEBUG
    std::cout << "Reading page "<<pageno<<endl;
#endif
...
Status DB::set_bits( PageId start_page, unsigned run_size, int bit )
{
    ...
    for ( PageId pgid=first_map_page; pgid <= last_map_page; ++pgid, first_bit_no=0 )
      {
        Status status;
            ...
            for ( unsigned mask=1;
                  ...
                  mask <<= 1, --num_bits_this_page, ++bit_number )
              {
                ...
                if ( bit_number % 10 == 0 )
                    if ( bit_number % 50 == 0 )
                      {
                        if ( bit_number ) std::cout << std::endl;
                        std::cout << std::setw(8) << bit_number << ": ";
                      }
                    else
                        std::cout << ' ';
                std::cout << bit;
