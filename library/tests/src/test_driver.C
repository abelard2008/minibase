                              ...
                              const char* activity, int postedErrExpected )
{
    if ( status == OK )
      {
        status = FAIL;
        std::cerr << "*** " << activity << " did not return a failure status.\n";
      }
    else if ( status != expectedStatus )
        std::cerr << "*** " << activity << " correctly returned a failure status,\n"
    ...
    if ( !result || minibase_errors.error() )
      {
        ...
        if ( minibase_errors.error() )
            std::cerr << (result? "*** Unexpected error(s) logged, test failed:\n"
                     : "Errors logged:\n");
...
Status TestDriver::runAllTests()
{
    ...
    testFunctionxo aa[6] = {&TestDriver::test1,&TestDriver::test2,&TestDriver::test3,&TestDriver::test4,&TestDriver::test5,&TestDriver::test6};

    for(i = 0; i < 6; i++) {
      int result = aa[i]();//test11();
      if ( !result || minibase_errors.error() )
      {
        status = FAIL;
        if ( minibase_errors.error() )
            std::cout << (result? "*** Unexpected error(s) logged, test failed:\n"
                     : "Errors logged:\n");
...
#if 0    
    Status answer = OK;
    runTest( answer, test1 );
    runTest( answer, test2 );
    runTest( answer, test3 );
    runTest( answer, test4 );
    runTest( answer, test5 );
    runTest( answer, test6 );
    return answer;
#endif
}
