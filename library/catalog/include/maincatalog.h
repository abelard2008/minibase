...
public:
    ...
    Status addIndex(const char* relation, const char* attrname, IndexType accessType, int buckets);

      // drop an index from a relation
    Status dropIndex(const char* relation, const char* attrname, IndexType accessType);

      // get a catalog entry for an attribute
    Status getAttributeInfo(const char *relation, const char *attrName, AttrDesc &record);

      // get catalog entries for all attributes of a relation
    Status getRelAttributes(const char *relation, int &attrCnt, AttrDesc *&attrs);
  
      // get catalog entries for all indexes for a relation
    Status getRelIndexes(const char* relation, int &indexCnt, IndexDesc *&indexes);

      // get catalog entries for all indexes for an attribute 
    Status getAttrIndexes(const char* relation, const char *attrName, int &indexCnt, IndexDesc *&indexes);
 
      // get catalog entry on an index
    Status getIndexInfo(const char* relation, const char* attrName, IndexType accessType, IndexDesc &record);
