				 ...
				 RID& rid)
{
  ...
  // general plan:
  //    1. Insert the record into the page, which is then not necessarily any more sorted
  //    2. Sort the page by rearranging the slots (insertion sort)

  status = HFPage::insertRecord(recPtr, recLen, rid);
  if (status != OK)
    RAISE_ERROR(SORTEDPAGE, INSERTRECFAILED);
  ...
  // ASSERTIONS:
  // - record keys increase with decreasing slot number (starting at slot 0)
...
Status SortedPage::deleteRecord (const RID& rid)
{
  ...
  if (status == OK)
    ...
    RAISE_ERROR(SORTEDPAGE, DELETERECFAILED);
