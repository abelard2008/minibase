/*
 * key.cc - implementation of <key,data> abstraction for BT*Page and 
 *          BTreeFile code.
 *
...
int get_key_length(const void *key, const AttrType key_type)
{
  ...
  case attrString:
    len = strlen((char *) key); 
    return len+1;
//    return (len == MAX_KEY_SIZE1 ? len : len + 1);  // one for nul, if space allows
// XXX this should really be the conditional stmt, not just len+1 
    break;
  default:
    assert(0);
    return 0;
