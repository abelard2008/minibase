...
Status BTIndexPage::deleteKey (const void *key, AttrType key_type, RID& curRid)
{
    ...
    curRid.slotNo *= -1; // due to different slot representation used by get_next and deleteRecord
...
bool BTIndexPage::get_sibling(const void *key, AttrType key_type, PageId &pageNo, int &left)
{
...
Status BTIndexPage::adjust_key(const void *newKey, const void *oldKey, AttrType key_type)
{
...
bool BTIndexPage::redistribute(BTIndexPage *pptr, BTIndexPage *parentPtr, AttrType key_type, int left, const void *deletedKey)
{
    ...
    if (left) { // 'this' is the left sibling of pptr
        if (slot[slotCnt+1].length + free_space() > (MAX_SPACE-DPFIXED)/2) {
            ...
            st = pptr->insertKey((void*)&entry, key_type, leftMostPageId, dummyRid);
            if (st != OK)
                return false;
            ...
            if (deletedKey)
                st = parentPtr->adjust_key((void*)&lastKey, deletedKey, key_type);
            else 
                st = parentPtr->adjust_key((void*)&lastKey, (void*)&oldKey, key_type);
            assert (st == OK);
        }
    }
        ...
        if (slot[0].length + free_space() > (MAX_SPACE-DPFIXED)/2) {
            ...
            st = parentPtr->adjust_key((void*)&firstKey, (void*)&entry, key_type);
            assert(st == OK);
        }
    }
