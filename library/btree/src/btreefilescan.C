...
Status BTreeFileScan::get_next (RID & rid, void* keyptr)
{
  ...
  if (endkey && keyCompare(keyptr, endkey, treep->headerPage->key_type) > 0) {
    ...
    st = MINIBASE_BM->unpinPage(leafp->page_no());
    CHECK_ADD_ERROR(st, BTREE, BTreeFile::CANT_UNPIN_PAGE);
    return DONE;
  }
...
Status BTreeFileScan::delete_current ()
{
  ...
  if (leafp == NULL) {
    RAISE_ERROR(BTREE, BTreeFile::INVALID_SCAN);
  }

  st = MINIBASE_BM->pinPage(leafp->page_no(), (Page *&) dupPagePtr);
  CHECK_RAISE_ERROR(st, BTREE, BTreeFile::CANT_PIN_PAGE);
  ...
  if (st != OK) {
    ADD_ERROR(BTREE, BTreeFile::DELETE_CURRENT_FAILED);
  ...
  CHECK_RAISE_ERROR(st, BTREE, BTreeFile::CANT_UNPIN_PAGE);
