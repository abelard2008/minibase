			      ...
			      RID& rid)
{
  ...
  if (SortedPage::insertRecord(key_type, (char*)&entry,
			       entry_len, rid) != OK) {
    RAISE_ERROR(BTLEAFPAGE, LEAFINSERTRECFAILED);
    /*NOTREACHED*/
  }
...
Status BTLeafPage::get_data_rid(const void *key,
				...
				RID & dataRid)
{
  int lower = slotCnt+1;      // vars used for binary search
  int upper = 0;
  int mid;
  int result;
...
{ 
  ...
  if (slotCnt == 0) {
    ...
    dataRid.slotNo = -1;  // there's no `INVALID_SLOT' symbol so we use -1
    return NOMORERECS;
  }
...
bool BTLeafPage::redistribute(BTLeafPage *pptr, BTIndexPage *parentPtr, AttrType key_type, int left, const void *deletedKey)
{
    ...
    if (left) { // 'this' is the left sibling of pptr
        ...
        if (slot[slotCnt+1].length + free_space() > (MAX_SPACE-DPFIXED)/2) {
            ...
            // insert it into its sibling
            RID dummyRid, dummydummyRid;
            Status st = pptr->insertRec((void*)&firstKey, key_type, firstRid, dummyRid);
            if (st != OK)
                return false;
            ...
            // get the current first record of the old page for adjusting parent pointer
            Keytype newKey;
            st = get_first(dummyRid, (void*)&newKey, dummydummyRid); 
            assert(st == OK);
            
            // adjust the entry pointing to itself in its parent
            st = parentPtr->adjust_key((void*)&newKey, (void*)&firstKey, key_type);
            assert(st == OK);
        }
    }
