...
// ------------------ get_data_rid -----------------------
// This function performs a sequential search to look for the
// rid of the data record. dataRid contains the RID of
// the DATA record NOT the rid of the data entry!
// (returns RECNOTFOUND if dataRid can not BE found)

//   Status get_data_rid(const void *key, AttrType key_type, RID & dataRid);
...
// HFPage::nextRecord(), and break the flat record into its
// two components: namely, the key and datarid.
// return NOMORERECS when at end of iteration
  ...
  bool redistribute (BTLeafPage* page, BTIndexPage* parentpage, AttrType key_type, int left, const void *deleted_key);
