 ...
 *
 * Gideon Glass & Johannes Gehrke  951022  CS564  UW-Madison
 */
...
//#define BT_TRACE
  /* When #defined, BT_TRACE causes a structured trace to be written to the
     std::ostream pointed to by the class variable BTreeFile::Trace.  This output is
     ...
     std::ostream of your choice before creating your BTreeFile. */
...
class BTreeFile: public IndexFile
{
   ...
   */
    ...
    const int DPFIXED = sizeof(slot_t)+4*sizeof(short)+3*sizeof(PageId);
    ...
    int keysize;	// stored max key length (specified at index creation)
...
public:
  ...
  BTreeFile(Status& status, std::string filename);
  // an index with given filename should already exist; this opens it.
  
  BTreeFile(Status& status, std::string filename, const AttrType keytype,
...
private:
  ...
  char *dbname;          // copied from arg of the ctor; needed for Delete
  ...
  /*
   * _destoyFile: recursively destroy the tree rooted at a specified page.
   */
