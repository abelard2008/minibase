...
#include "logrecord.h"  //  class logrecord
#include "logshm.h"     //  shared memory structures
#include "rawfile.h"    //  declaration of the Rawfile class
#include "logerror.h"   //  definition of errors
...
/*
   ...
   read
        Reads a log record given the LSN.  Returns true on success,
        false on failure.
   ...
   find_log_size
        Find the size of the log used so far.   
...
public:
	...
	// testing purposes
	//void restart(void);
	//lsn_t getfirstlsn(void);
	Rawfile *logfile_ptr;   // dont know if this needs to be 
				// deleted too.
};
