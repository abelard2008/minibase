 ...
 *
 *      AttrValue                stores either a string, integer, or real
 *
...
enum AttrOperator { aopEQ, aopLT, aopGT, aopNE, aopLE, aopGE, aopNOT, aopNOP, aopRANGE };
...
public:
  ...
  int operator==(const PropertyNode & pn) 
    { return (name == pn.name && data == pn.data); };
};
 ...
 * string, an integer, or a real.  The class Select (declared in this file)
 ...
 *
 *      s                        stores a std::string if type is attrString
 *
 ...
 *
 *      MakeString               returns a std::string representation of AttrValue;
...
class AttrValue
{
  private:
    AttrType type;
    std::string s;    // left out of the union because the compiler didn't like it
    union {
	long i;
	double r;
    };
  ...
  public:
    ...
    friend std::ostream & operator << (std::ostream & s, const AttrValue & av);
    std::string MakeString();
    
    void Read(std::istream & s, AttrType type);
};
 ...
 */ 
  ...
  public:
    Attribute(){};
 Attribute(const std::string& name) : name(name), attrId(0), 
    ...
    Attribute(const std::string & iName, Plan_Relation *iRelation, AttrType iType,
...
struct OrderAttr
{
    ...
    bool operator==(const OrderAttr &a) {return attr == a.attr && tupleOrder == a.tupleOrder; } ;
 ...
 *
 *      Plan_AccessMethod(const std::string & iName, const double & iCost, 
...
public:
  ...
  Plan_AccessMethod();
  Plan_AccessMethod(const std::string & iName,
  ...
  std::string GetDescription(bool indexOnly = false);
 ...
 *
 *      Plan_Relation(const std::string & iName, DataBase *iDb, ulong iCardinality,
...
protected:
  ...
  std::string name;
  std::string originalName; // this name is the "real" one; when read in
  DataBase *db;        
  ulong cardinality;
  int tupleSize;
  int numPages;
...
public:
  ...
  std::string OriginalName() { return originalName;}	 // this is the "real" one    
  ...
  void print() { std::cout << "<Plan Relation " <<  name; };
  bool operator==(const Plan_Relation & r) { return (originalName == r.originalName); };
  bool operator!=(const Plan_Relation & r) { return (originalName != r.originalName); };
};
 ...
 * stores either a string, an integer, or a float) or a pointer to an
 ...
 *
 *      MakeString               returns a std::string representation of the 
 *                               select operand
 */
class SelectOperand
{
...
public:
  ...
  int operator==(const Attribute & attrib) {
                        if (!literal && a) return (*a == attrib); 
                        else return FALSE; };
  
  int operator==(const SelectOperand & so);
  int operator!=(const SelectOperand & so) { return !(*this == so);};
 ...
 *      MakeString               returns a std::string representation of 
 *                               the class
 */
class SelectElem
{
...
public:    
    ...
    SelectElem() {arg1 = arg2 = arg3 = NULL; op = op2 = SelectElem::OPTRUE;};
    SelectElem(AttrOperator iOp, SelectOperand *iArg1, SelectOperand *iArg2);
    ...
    ~SelectElem() { if (arg1) delete arg1; if (arg2) delete arg2; 
		    if (arg3) delete arg3; } ;
 ...
 *
 * GetSelectivity and MakeString do the same as the members of SelectElem 
...
public:
  ...
  bool Contains(Attribute *a) { if (isElem && elem) return elem->Contains(a);
                                else if (term) return term->Contains(a);
				else return false; 
			    };
  				
  double GetSelectivity();

  ~SelectExpr() { if (isElem && elem) delete elem; 
                  else if (term) delete term; } ;
...
public:
  ...
  std::string MakeString() { if (expr) return expr->MakeString(); else return ""; };
};
...
public:
  ...
  State(): joins(), project(), neededAttributes(), selects(), groupby(), having(), omits(), order()
  {distinct=false; containsAggregates=false; aggDistinct=false; };
